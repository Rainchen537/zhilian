...
ali_get_record() {
  local body
  body=$(ali_api "Action=DescribeDomainRecords
DomainName=${ZONE}
PageSize=100
...
ali_list_matching_records() {
  ...
  if has_cmd jq; then
    jq -r --arg rr "$RR" --arg type "$RECORD_TYPE" '
      ...
      | .[]
      | [.RecordId, .Value, (.Line // "default")] | @tsv
    ' <<<"$body"
  else
    python3 - "$RR" "$RECORD_TYPE" <<'PY' <<<"$body"
import json, sys
rr = sys.argv[1]
rtype = sys.argv[2]
try:
...
for rec in records:
    if (rec.get('RR') == rr) and (rec.get('Type') == rtype):
        print(f"{rec.get('RecordId','')}\t{rec.get('Value','')}\t{rec.get('Line','default')}")
PY
  fi
}
...
ali_upsert_record() {
  local current_ip="$1"
  local body full rr_for_api add_body
  local match_count=0 same_count=0 updated_count=0
  local record_id remote_ip record_line update_body

  body=$(ali_get_record) || die "Aliyun record query failed"
  full=$(full_record_name)
  rr_for_api="$RR"

  while IFS=$'\t' read -r record_id remote_ip record_line; do
    [[ -n "$record_id" ]] || continue
    match_count=$((match_count + 1))

    if [[ "$remote_ip" == "$current_ip" ]]; then
      same_count=$((same_count + 1))
      debug "Aliyun existing match: id=${record_id}, line=${record_line}, value=${remote_ip}"
      continue
    fi
...
Value=${current_ip}
TTL=${TTL}
Line=${record_line}") || die "Aliyun update request failed"
    if grep -q '"RecordId"' <<<"$update_body"; then
      updated_count=$((updated_count + 1))
      log "Aliyun: updated ${full} [line=${record_line}] -> ${current_ip}"
    else
      die "Aliyun update failed: $update_body"
    fi
  done < <(ali_list_matching_records "$body")

  if (( match_count > 0 )); then
    if (( match_count > 1 )); then
      log "WARN: Aliyun: found ${match_count} existing ${RECORD_TYPE} records for ${full}; updated ${updated_count}, already-current ${same_count}"
    elif (( updated_count == 0 )); then
